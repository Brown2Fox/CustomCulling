...
namespace RootMotion.Dynamics {
	...
	public class PuppetMasterInspector : Editor {
		private PuppetMaster script {
			get { return target as PuppetMaster; }
		}
		...
		public override void OnInspectorGUI() {
			...
			for (var i = 0; i < script.muscles.Length; i++) {
				Muscle m = script.muscles[i];
				if (m.joint == null) {
					script.muscles[i].name = "Missing Joint Reference!";
				} else if (m.target == null) {
					script.muscles[i].name = "Missing Target Reference!";
