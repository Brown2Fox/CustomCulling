...
namespace RootMotion.Dynamics {
	...
	public struct Muscle {
		...
		public struct Props {
			...
			/// </summary>
			public Props(float i) {
				mappingWeight = 1f;
				pinWeight = 1f;
				muscleWeight = 1f;
				muscleDamper = 1f;
				animatedTargetChildren = new Transform[0];
				internalCollisionIgnores = new InternalCollisionIgnoreSettings();
				group = default;
			}
		...
		*/
		...
		public void ResetInternalCollisions(Muscle m, bool useInternalCollisionIgnores) {
			...
			foreach (Collider c in colliders) {
				if (c && c.enabled && c.gameObject.activeInHierarchy)
					foreach (Collider c2 in m.colliders) {
						if (c2 && c2.enabled && c2.gameObject.activeInHierarchy) {
							Physics.IgnoreCollision(c, c2, forceIgnore);
						}
					}
			}
		}
		...
		// Update this Muscle
		public void Update(UpdateParams _updateParams) {
			...
			if (_updateParams.rotationTargetChanged) MuscleRotation(_updateParams);

			//offset = Vector3.Lerp(offset, Vector3.zero, Time.deltaTime * 5f);
		}


		public static void StaticUpdate(Muscle[] _muscles, UpdateParams _updateParams) {
			for (int i = 0; i < _muscles.Length; i++) {
				var state = _muscles[i].state;
				state.velocity = _muscles[i].rigidbody.velocity;
				state.angularVelocity = _muscles[i].rigidbody.angularVelocity;
				...
				_muscles[i].state = state;
				_muscles[i].props.Clamp();
			}

			StaticPin(_muscles, _updateParams);

			if (_updateParams.rotationTargetChanged) {
				StaticMuscleRotation(_muscles, _updateParams.rotateParams);
			}
		}


		// Add force to the rigidbody to make it match the target position
		private static void StaticPin(Muscle[] _muscles, UpdateParams _updateParams) {
			for (int i = 0; i < _muscles.Length; i++) {
				var posOffset = _muscles[i].targetAnimatedCenterOfMass - _muscles[i].rigidbody.worldCenterOfMass;
				if (float.IsNaN(posOffset.x)) posOffset = Vector3.zero;
				_muscles[i].positionOffset = posOffset;

				float w = _updateParams.pinParams.pinWeightMaster * _muscles[i].props.pinWeight * _muscles[i].state.pinWeightMlp;
				if (w <= 0f) return;

				w = Mathf.Pow(w, _updateParams.pinParams.pinPow);
				{
					float offsetSqrDist = posOffset.sqrMagnitude;
					if (!float.IsInfinity(_updateParams.invDeltaTime)) posOffset *= _updateParams.invDeltaTime;

					Vector3 force = -_muscles[i].rigidbody.velocity + _muscles[i].targetVelocity + posOffset;

					if (_muscles[i].rigidbody.useGravity) force += _updateParams.minusGravity;

					force *= w;

					if (_updateParams.pinParams.pinDistanceFalloff > 0f) force /= 1f + offsetSqrDist * _updateParams.pinParams.pinDistanceFalloff;

					_muscles[i].rigidbody.AddForce(force, ForceMode.VelocityChange);
				}
				if (_updateParams.pinParams.angularPinning) {
					Vector3 torque = PhysXTools.GetAngularAccelerationNoTime(_muscles[i].rigidbody.rotation, _muscles[i].defaultMuscleRotRelToTarget * _muscles[i].targetAnimatedWorldRotation);
					torque *= _updateParams.invDeltaTime;

					torque -= _muscles[i].rigidbody.angularVelocity;
					torque *= w;
					_muscles[i].rigidbody.AddTorque(torque, ForceMode.VelocityChange);
				}

				if (_muscles[i].additionalPin != null) {
					Vector3 additionalPositionOffset = Vector3.zero;
					additionalPositionOffset = _muscles[i].additionalPinTargetAnimatedCenterOfMass - _muscles[i].additionalRigidbody.worldCenterOfMass;

					if (float.IsNaN(additionalPositionOffset.x)) additionalPositionOffset = Vector3.zero;

					var force = -_muscles[i].additionalRigidbody.velocity + _muscles[i].additionalTargetVelocity + additionalPositionOffset;
					force *= w * _muscles[i].additionalPinWeight;

					if (_muscles[i].additionalRigidbody.useGravity) force += _updateParams.minusGravity;

					if (_updateParams.pinParams.pinDistanceFalloff > 0f) force /= 1f + additionalPositionOffset.sqrMagnitude * _updateParams.pinParams.pinDistanceFalloff;

					_muscles[i].additionalRigidbody.AddForce(force, ForceMode.VelocityChange);
				}
			}
		}

		// Apply Joint targetRotation to match the target rotation
		private static void StaticMuscleRotation(Muscle[] _muscles, RotateParams _rotateParams) {
			for (int i = 0; i < _muscles.Length; i++) {
				float w = _rotateParams.muscleWeightMaster * _muscles[i].props.muscleWeight * _rotateParams.muscleSpring * _muscles[i].state.muscleWeightMlp * 10f;

				// If no connection point, don't rotate;
				if (_muscles[i].joint.connectedBody == null) w = 0f;
				else if (w > 0f) _muscles[i].joint.targetRotation = _muscles[i].toJointSpaceInverse * Quaternion.Inverse(_muscles[i].targetAnimatedRotation) * _muscles[i].toJointSpaceDefault;

				float d = (_muscles[i].props.muscleDamper * _rotateParams.muscleDamper * _muscles[i].state.muscleDamperMlp) + _muscles[i].state.muscleDamperAdd;

				if (w == _muscles[i].lastJointDriveRotationWeight && d == _muscles[i].lastRotationDamper) continue;

				_muscles[i].lastJointDriveRotationWeight = w;

				_muscles[i].lastRotationDamper = d;
				_muscles[i].slerpDrive.positionSpring = w;
				_muscles[i].slerpDrive.maximumForce = Mathf.Max(w, d) * _muscles[i].state.maxForceMlp;
				_muscles[i].slerpDrive.positionDamper = d;

				_muscles[i].joint.slerpDrive = _muscles[i].slerpDrive;
			}
		}

		public struct UpdateParams {
			public PinParams pinParams;
			public RotateParams rotateParams;
			public float invDeltaTime;
			public bool rotationTargetChanged;
			public Vector3 minusGravity;
		}

		public struct PinParams {
			public float pinWeightMaster;
			public float pinPow;
		...
		public struct RotateParams {
			public float muscleWeightMaster;
		...
		public float lastJointDriveRotationWeight;
		public float lastRotationDamper;
		...
		public Quaternion targetAnimatedRotation;
		private Quaternion defaultRotation,
						   rotationRelativeToTarget,
						   defaultTargetLocalRotation,
						   lastMappedRotation;
		...
		private void Pin(in UpdateParams _updateParams) {
			...
			float w = _updateParams.pinParams.pinWeightMaster * props.pinWeight * state.pinWeightMlp;
			if (w <= 0f) return;

			w = Mathf.Pow(w, _updateParams.pinParams.pinPow);
			Pin(rigidbody, positionOffset, targetVelocity, w, _updateParams);

			if (_updateParams.pinParams.angularPinning) {
				Vector3 torque = PhysXTools.GetAngularAccelerationNoTime(rigidbody.rotation, defaultMuscleRotRelToTarget * targetAnimatedWorldRotation);
				torque *= _updateParams.invDeltaTime;
			...
			if (additionalPin != null) {
				...
				Pin(additionalRigidbody, additionalPositionOffset, additionalTargetVelocity, w * additionalPinWeight, _updateParams);
			}
		}

		private void Pin(Rigidbody r, Vector3 posOffset, Vector3 targetVel, float w, in UpdateParams _updateParams) {
			Vector3 p = posOffset;
			if (!float.IsInfinity(_updateParams.invDeltaTime)) p *= _updateParams.invDeltaTime;

			Vector3 force = -r.velocity + targetVel + p;
			force *= w;

			if (r.useGravity) force += _updateParams.minusGravity;

			if (_updateParams.pinParams.pinDistanceFalloff > 0f) force /= 1f + posOffset.sqrMagnitude * _updateParams.pinParams.pinDistanceFalloff;

			r.AddForce(force, ForceMode.VelocityChange);
		}

		// Apply Joint targetRotation to match the target rotation
		private void MuscleRotation(in UpdateParams _updateParams) {
			//Profiler.BeginSample($"Muscle.Rotation");
			float w = _updateParams.rotateParams.muscleWeightMaster * props.muscleWeight * _updateParams.rotateParams.muscleSpring * state.muscleWeightMlp * 10f;

			// If no connection point, don't rotate;
			if (joint.connectedBody == null) w = 0f;
			else if (w > 0f) joint.targetRotation = LocalToJointSpace(targetAnimatedRotation);

			float d = (props.muscleDamper * _updateParams.rotateParams.muscleDamper * state.muscleDamperMlp) + state.muscleDamperAdd;

			if (w == lastJointDriveRotationWeight && d == lastRotationDamper) {
				//Profiler.EndSample();
				return;
			}
		...
		private Quaternion localRotation {
			get { return Quaternion.Inverse(parentRotation) * transform.rotation; }
		}
		...
		private Quaternion targetLocalRotation {
			get { return Quaternion.Inverse(targetParentRotation * toParentSpace) * target.rotation; }
		}
		...
		public static bool operator ==(Muscle m1, Muscle m2) {
			return m1.target == m2.target && m1.rigidbody == m2.rigidbody;
		}
