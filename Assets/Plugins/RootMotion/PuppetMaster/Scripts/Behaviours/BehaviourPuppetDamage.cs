...
namespace RootMotion.Dynamics {
	...
	public partial class BehaviourPuppet : BehaviourBase {
		...
		protected override void OnMuscleHitBehaviour(MuscleHit hit) {
			...
			BehaviourPuppet.MuscleProps props = GetProps(puppetMaster.muscles[hit.muscleIndex].props.group);


			// Add force
			puppetMaster.muscles[hit.muscleIndex].SetKinematic(false);
			puppetMaster.muscles[hit.muscleIndex].rigidbody.AddForceAtPosition(hit.force * props.hitImpulseMultiplier, hit.position);
		}
		...
		private void UnPinMuscle(int muscleIndex, float unpin) {
			...
			if (!puppetMaster.muscles[muscleIndex].state.isDisconnected) {
				puppetMaster.muscles[muscleIndex].state.pindelay = props.pinDelayOnHit;
