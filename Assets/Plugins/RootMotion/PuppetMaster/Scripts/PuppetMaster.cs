...
namespace RootMotion.Dynamics {
	...
	public partial class PuppetMaster : MonoBehaviour {
		...
		public Transform targetMover; // { get; private set; }
		...
		public UpdateMode updateMode {
			get { return targetUpdateMode == AnimatorUpdateMode.AnimatePhysics ? (isLegacy ? UpdateMode.AnimatePhysics : UpdateMode.FixedUpdate) : UpdateMode.Normal; }
		}
		...
		void OnDisable() {
			if (!gameObject.activeInHierarchy && initiated && Application.isPlaying) {
				for (var i = 0; i < muscles.Length; i++) muscles[i].Reset();
			}
		...
		void OnEnable() {
			if (gameObject.activeInHierarchy && initiated && hasBeenDisabled && Application.isPlaying) {
				...
				for (var i = 0; i < muscles.Length; i++) {
					Muscle.State s = muscles[i].state;
					s.pinWeightMlp = state == State.Alive ? 1f : 0f;
					s.muscleWeightMlp = state == State.Alive ? 1f : stateSettings.deadMuscleWeight;
					s.muscleDamperAdd = 0f;
					//m.state.immunity = 0f;
					muscles[i].state = s;
				}
				...
				if (state != State.Frozen && mode != Mode.Disabled) {
					...
					// Deactivate/Freeze
					for (var i = 0; i < muscles.Length; i++) muscles[i].joint.gameObject.SetActive(false);
		...
		*/
		...
		private void Initiate() {
			...
			if (!targetMover) targetMover = transform.parent.parent;
			...
			for (int i = 0; i < muscles.Length; i++) {
				...
				if (behaviours.Length > 0) {
					muscle.broadcaster = muscle.joint.gameObject.GetComponent<MuscleCollisionBroadcaster>();
					if (muscle.broadcaster == null) muscle.broadcaster = muscle.joint.gameObject.AddComponent<MuscleCollisionBroadcaster>();
					muscle.broadcaster.puppetMaster = this;
					muscle.broadcaster.muscleIndex = i;
				}

				muscle.jointBreakBroadcaster = muscle.joint.gameObject.GetComponent<JointBreakBroadcaster>();
				if (muscle.jointBreakBroadcaster == null) muscle.jointBreakBroadcaster = muscle.joint.gameObject.AddComponent<JointBreakBroadcaster>();
				muscle.jointBreakBroadcaster.puppetMaster = this;
				muscle.jointBreakBroadcaster.muscleIndex = i;
		...
		private void OnRebuild() {
			...
			for (var i = 0; i < defaultMuscles.Length; i++) {
				defaultMuscles[i].Rebuild();
			}

			foreach (Muscle m in defaultMuscles) {
				if (!ContainsJoint(m.joint)) {
					AddMuscle(m.joint, m.target, m.rebuildConnectedBody, m.rebuildTargetParent, new Muscle.Props());
				}
			}
		...
		public void OnPreSimulate(float deltaTime) {
			...
			if (!isFrozen) {
				...
				*/
				...
				Profiler.BeginSample("PreSimulate MuscleUpdates");
				// Update Muscles
				// for (int i = 0; i < muscles.Length; i++) {
				// 	muscles[i].Update();
				// }

				Muscle.UpdateParams updateParams;

				updateParams.invDeltaTime = 1f / Time.fixedDeltaTime;
				updateParams.minusGravity = Physics.gravity * -Time.fixedDeltaTime;
				updateParams.rotationTargetChanged = true;

				updateParams.pinParams.pinWeightMaster = pinWeight;
				updateParams.pinParams.pinPow = pinPow;
				updateParams.pinParams.pinDistanceFalloff = pinDistanceFalloff;
				updateParams.pinParams.angularPinning = angularPinning;

				updateParams.rotateParams.muscleWeightMaster = muscleWeight;
				updateParams.rotateParams.muscleSpring = muscleSpring;
				updateParams.rotateParams.muscleDamper = muscleDamper;


				Muscle.StaticUpdate(muscles, updateParams);
				Profiler.EndSample();
			}
...
// Fix transforms to be sure of not having any drifting when the target bones are not animated
		...
		public void OnPostSimulate() {
			...
			if (initiated) {
				...
				if (!isFrozen) {
					...
					for (var i = 0; i < muscles.Length; i++) {
						muscles[i].CalculateMappedVelocity();
					}
				}
			...
			if (onPostRebuildFlag) {
				...
				for (var i = 0; i < muscles.Length; i++) {
					muscles[i].MoveToTarget();
					muscles[i].ClearVelocities();
				}
		...
		protected virtual void FixedUpdate() {
			// Debug.Log("Puppet Fixed");
			Profiler.BeginSample("BehavioursUpdates");
...
// If updating the Animator manually here in FixedUpdate
			...
			if (!isFrozen) {
				...
				*/
				...
				//Profiler.BeginSample("MuscleUpdates");
				// Update Muscles
				// for (int i = 0; i < muscles.Length; i++) {
				// 	muscles[i].Update();
				// }
		...
		private Muscle.UpdateParams updateParams;
...
// Teleporting
			...
			if (isAlive && updateJointAnchors) {
				...
				muscles[0].joint.connectedAnchor = muscles[0].target.position;
			}
		}
			...
			for (var i = 0; i < muscles.Length; i++) {
				...
				if (m.joint.connectedBody != null && m.connectedBodyTarget != null) {
					...
					for (var j = 0; j < muscles.Length; j++) {
						Muscle m2 = muscles[j];
						if (i != j && m2.joint.connectedBody == m.rigidbody) {
							isEndMuscle = false;
							break;
						}
					}
