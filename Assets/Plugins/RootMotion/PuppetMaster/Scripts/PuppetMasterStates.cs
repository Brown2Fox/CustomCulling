	...
	public partial class PuppetMaster : MonoBehaviour {
		...
		public struct StateSettings {
			...
			public static StateSettings Default {
				get { return new StateSettings(1f, 0.01f, 2f, 0.02f, false, true, true); }
			}
		}
		...
		/// </summary>
		public bool isAlive {
			get { return activeState == State.Alive; }
		}
		...
		public bool isFrozen {
			get { return activeState == State.Frozen; }
		}
		...
		private IEnumerator AliveToDead(bool freeze) {
			...
			if (stateSettings.killDuration > 0f && range > 0f) {
				...
				while (mW > stateSettings.deadMuscleWeight) {
					...
					for (var i = 0; i < muscles.Length; i++) {
						muscles[i].state.muscleWeightMlp = mW;
					}
			...
			for (var i = 0; i < muscles.Length; i++) {
				muscles[i].state.muscleWeightMlp = stateSettings.deadMuscleWeight;
			}
