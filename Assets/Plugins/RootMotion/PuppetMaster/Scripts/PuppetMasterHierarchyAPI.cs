...
namespace RootMotion.Dynamics {
	...
	public partial class PuppetMaster : MonoBehaviour {
		...
		/// </summary>
		public void AddMuscle(ConfigurableJoint joint, Transform target, Rigidbody connectTo, Transform targetParent, Muscle.Props muscleProps, bool forceTreeHierarchy = false, bool forceLayers = true) {
		...
		private void OnReconnectMuscleRecursive(int index) {
			...
			if (index == 0) {
				...
				for (var i = 0; i < muscles.Length; i++) {
					if (muscles[i].isPropMuscle) continue;
					muscles[i].state.isDisconnected = false;
