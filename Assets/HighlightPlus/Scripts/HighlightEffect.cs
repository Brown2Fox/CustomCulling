...
namespace HighlightPlus {
	...
	public partial class HighlightEffect : MonoBehaviour {
		public string colorInShader = "_Color";
		...
		Material fxMatOutline {
			get {
				if (_fxMatOutline == null && fxMatOutlineRef != null) {
					...
					if (useGPUInstancing) _fxMatOutline.enableInstancing = true;
					else _fxMatOutline.enableInstancing = false;
				}
		...
		Material fxMatGlow {
			get {
				if (_fxMatGlow == null && fxMatGlowRef != null) {
					...
					if (useGPUInstancing) _fxMatGlow.enableInstancing = true;
					else _fxMatGlow.enableInstancing = false;
				}
						...
						for (int l = 0; l < mesh.subMeshCount; l++) {
							...
							if (outlineQuality == QualityLevel.High) {
								for (int o = 0; o < offsets.Length; o++) {
									...
									mat.SetVector(OutlineDirectionPropID, direction);
									if (rms[k].preserveOriginalMesh) {
										cbOutline.Clear();
				...
				if (rms[k].preserveOriginalMesh) {
					...
					for (int l = 0; l < mesh.subMeshCount; l++) {
						...
						if (_highlighted && ((outline > 0 && smoothOutlineVisibility != Visibility.Normal) || (glow > 0 && smoothGlowVisibility != Visibility.Normal) || (innerGlow > 0 && innerGlowVisibility != Visibility.Normal))) {
							rms[k].fxMatMask[l].SetInt(ZTestPropID, (int) UnityEngine.Rendering.CompareFunction.Always);
						} else {
							rms[k].fxMatMask[l].SetInt(ZTestPropID, (int) UnityEngine.Rendering.CompareFunction.LessEqual);
						}
			...
			for (int k = 0; k < rmsCount; k++) {
					...
					if (overlayColor.a > 0) {
						for (int l = 0; l < mesh.subMeshCount; l++) {
							...
							if (l < rms[k].fxMatOverlay.Length && rms[k].fxMatOverlay[l] != null) {
								...
								fxMat.SetVector(OverlayDataPropID, new Vector3(overlayAnimationSpeed, overlayMinIntensity, overlayBlending));
				...
				for (int l = 0; l < mesh.subMeshCount; l++) {
					...
					if (glow > 0 && glowQuality != QualityLevel.Highest) {
						...
						mat.SetVector(GlowDirectionPropID, Vector3.zero);
						for (int j = 0; j < glowPasses.Length; j++) {
							Color dataColor = glowPasses[j].color;
							mat.SetColor(GlowColorPropID, dataColor);
							...
							mat.SetVector(GlowPropID, dataGlow);
							if (glowQuality == QualityLevel.High) {
								for (int o = 0; o < offsets.Length; o++) {
									Vector3 direction = offsets[o];
									direction.y *= cam.aspect;
									mat.SetVector(GlowDirectionPropID, direction);
									if (rms[k].preserveOriginalMesh) {
										cbGlow.Clear();
						...
						if (useGPUInstancing) {
							...
							if (instanceCount > 0) {
								glowPropertyBlock.SetVectorArray(GlowDirectionPropID, matDataDirection);
								glowPropertyBlock.SetVectorArray(GlowColorPropID, matDataColor);
								glowPropertyBlock.SetVectorArray(GlowPropID, matDataGlow);
					...
					if (outline > 0 && outlineQuality != QualityLevel.Highest) {
						...
						Material mat = fxMatOutline;
						mat.SetColor(OutlineColorPropID, outlineColor);
						if (outlineQuality == QualityLevel.High) {
							matDataDirection.Clear();
							for (int o = 0; o < offsets.Length; o++) {
								Vector3 direction = offsets[o] * (outlineWidth / 100f);
							...
							if (useGPUInstancing) {
								int instanceCount = matDataDirection.Count;
								if (instanceCount > 0) {
									outlinePropertyBlock.SetVectorArray(OutlineDirectionPropID, matDataDirection);
									if (matrices == null || matrices.Length < instanceCount) {
										matrices = new Matrix4x4[instanceCount];
									}
					...
					if (innerGlow > 0 && innerGlowWidth > 0) {
						if (l < rms[k].fxMatInnerGlow.Length && rms[k].fxMatInnerGlow[l] != null) {
							...
							rms[k].fxMatInnerGlow[l].SetColor(colorInShader, innerGlowColorA);
		...
		void SmoothGlow(int rtWidth, int rtHeight) {
			...
			if (glowDesc.vrUsage == VRTextureUsage.TwoEyes) {
				...
				fxMatBlurGlow.SetFloat(StereoRenderingPropID, 0.5f);
				fxMatComposeGlow.SetFloat(StereoRenderingPropID, 0.5f);
			} else {
				fxMatBlurGlow.SetFloat(StereoRenderingPropID, 1f);
				fxMatComposeGlow.SetFloat(StereoRenderingPropID, 1f);
			}
		...
		void SmoothOutline(int rtWidth, int rtHeight) {
			...
			if (outlineDesc.vrUsage == VRTextureUsage.TwoEyes) {
				...
				fxMatBlurOutline.SetFloat(StereoRenderingPropID, 0.5f);
				fxMatComposeOutline.SetFloat(StereoRenderingPropID, 0.5f);
			} else {
				fxMatBlurOutline.SetFloat(StereoRenderingPropID, 1f);
				fxMatComposeOutline.SetFloat(StereoRenderingPropID, 1f);
			}
		...
		void ComposeSmoothBlend(Visibility smoothGlowVisibility, Visibility smoothOutlineVisibility) {
			...
			if (renderSmoothGlow) {
				...
				matComposeGlow.SetVector(FlipPropID, (UnityEngine.XR.XRSettings.enabled && flipY) ? new Vector3(1, -1, 0) : new Vector3(0, 1, 0));
				if (glowOptimalBlit) {
					cbSmoothBlend.SetRenderTarget(BuiltinRenderTextureType.CameraTarget);
					matComposeGlow.SetInt(ZTestPropID, GetZTestValue(smoothGlowVisibility));
					matComposeGlow.SetColor(DebugPropID, glowBlitDebug ? debugColor : blackColor);
			...
			if (renderSmoothOutline) {
				...
				matComposeOutline.SetVector(FlipPropID, (UnityEngine.XR.XRSettings.enabled && flipY) ? new Vector3(1, -1, 0) : new Vector3(0, 1, 0));
				if (outlineOptimalBlit) {
					cbSmoothBlend.SetRenderTarget(BuiltinRenderTextureType.CameraTarget);
					matComposeOutline.SetInt(ZTestPropID, GetZTestValue(smoothOutlineVisibility));
					matComposeOutline.SetColor(DebugPropID, outlineBlitDebug ? debugColor : blackColor);
		...
		void SetupMaterial() {
			...
			switch (effectGroup) {
				...
				case TargetOptions.RootToChildren:
					...
					renderersList = new List<Renderer>(rr);
					for (int i = renderersList.Count - 1; i >= 0; --i)
						if (renderersList[i].GetComponent<TextMeshPro>() != null)
							renderersList.RemoveAt(i);
					rr = new Renderer[renderersList.Count];
					for (int i = renderersList.Count - 1; i >= 0; --i)
						rr[i] = renderersList[i];
					break;
		...
		void UpdateMaterialPropertiesNow() {
			...
			if (useSmoothGlow) {
				CheckBlurCommandBuffer();
				fxMatComposeGlow.SetInt(CullPropID, cullBackFaces ? (int) UnityEngine.Rendering.CullMode.Back : (int) UnityEngine.Rendering.CullMode.Off);
				fxMatBlurGlow.SetFloat(BlurScalePropID, glowWidth / glowDownsampling);
				fxMatBlurGlow.SetFloat(SpeedPropID, glowAnimationSpeed);
			}

			bool useSmoothOutline = outline > 0 && outlineQuality == QualityLevel.Highest;
			if (useSmoothOutline) {
				CheckBlurCommandBuffer();
				fxMatComposeOutline.SetInt(CullPropID, cullBackFaces ? (int) UnityEngine.Rendering.CullMode.Back : (int) UnityEngine.Rendering.CullMode.Off);
				fxMatBlurOutline.SetFloat(BlurScalePropID, outlineWidth / outlineDownsampling);
			}
			...
			for (int k = 0; k < rmsCount; k++) {
				if (rms[k].mesh != null) {
					...
					matOutline.SetFloat(OutlineWidthPropID, scaledOutlineWidth);
					matOutline.SetVector(OutlineDirectionPropID, Vector3.zero);
					matOutline.SetInt(OutlineZTestPropID, GetZTestValue(outlineVisibility));
					matOutline.SetInt(CullPropID, cullBackFaces ? (int) UnityEngine.Rendering.CullMode.Back : (int) UnityEngine.Rendering.CullMode.Off);
					matOutline.SetFloat(ConstantWidthPropID, constantWidth ? 1.0f : 0);

					// Glow
					Material matGlow = fxMatGlow;
					matGlow.SetVector(Glow2PropID, new Vector3(outlineWidth / 100f, glowAnimationSpeed, glowDithering ? 0 : 1));
					matGlow.SetInt(GlowZTestPropID, GetZTestValue(glowVisibility));
					matGlow.SetInt(CullPropID, cullBackFaces ? (int) UnityEngine.Rendering.CullMode.Back : (int) UnityEngine.Rendering.CullMode.Off);
					matGlow.SetFloat(ConstantWidthPropID, constantWidth ? 1.0f : 0);
					if (glowBlendMode == GlowBlendMode.AlphaBlend) {
						fxMatComposeGlow.SetInt(SrcBlendPropID, (int) UnityEngine.Rendering.BlendMode.SrcAlpha);
						fxMatComposeGlow.SetInt(DstBlendPropID, (int) UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
					} else {
						fxMatComposeGlow.SetInt(SrcBlendPropID, (int) UnityEngine.Rendering.BlendMode.One);
						fxMatComposeGlow.SetInt(DstBlendPropID, (int) UnityEngine.Rendering.BlendMode.One);
					}
					...
					for (int l = 0; l < rms[k].mesh.subMeshCount; l++) {
						...
						if (rms[k].fxMatMask != null && rms[k].fxMatMask.Length > l) {
							...
							if (fxMat != null) {
								...
								if (useAlphaTest) {
									fxMat.SetFloat(CutOffPropID, alphaCutOff);
								...
								fxMat.SetInt(CullPropID, cullBackFaces ? (int) UnityEngine.Rendering.CullMode.Back : (int) UnityEngine.Rendering.CullMode.Off);
							}
						}
						...
						if (rms[k].fxMatSeeThroughInner != null && rms[k].fxMatSeeThroughInner.Length > l) {
							...
							if (fxMat != null) {
								fxMat.SetFloat(SeeThroughPropID, seeThroughIntensity);
								fxMat.SetFloat(SeeThroughNoisePropID, seeThroughNoise);
								fxMat.SetColor(SeeThroughTintColorPropID, seeThroughTintColor);
								fxMat.SetFloat(SeeThroughBorderWidthPropID, (seeThroughBorder * seeThroughBorderWidth) > 0 ? seeThroughBorderWidth / 100f : 0);
								fxMat.SetFloat(SeeThroughBorderConstantWidthPropID, constantWidth ? 1.0f : 0);
						...
						if (rms[k].fxMatOverlay != null && rms[k].fxMatOverlay.Length > l) {
							...
							if (fxMat != null) {
								...
								if (mat.HasProperty(colorInShader)) {
									fxMat.SetColor(OverlayBackColorPropID, mat.GetColor(colorInShader));
								} else {
									Debug.LogError($"There is no property {colorInShader} in material {mat.name}", gameObject);
								}
						...
						if (rms[k].fxMatInnerGlow != null && rms[k].fxMatInnerGlow.Length > l) {
							...
							if (fxMat != null) {
								...
								fxMat.SetFloat(WidthPropID, innerGlowWidth);
								fxMat.SetInt(InnerGlowZTestPropID, GetZTestValue(innerGlowVisibility));
								if (useAlphaTest) {
									fxMat.SetFloat(CutOffPropID, alphaCutOff);
									fxMat.EnableKeyword(SKW_ALPHACLIP);
								} else {
						...
						if (rms[k].fxMatSolidColor != null && rms[k].fxMatSolidColor.Length > l) {
							...
							if (fxMat != null) {
								...
								fxMat.SetInt(CullPropID, cullBackFaces ? (int) UnityEngine.Rendering.CullMode.Back : (int) UnityEngine.Rendering.CullMode.Off);
								fxMat.SetInt(ZTestPropID, GetZTestValue(useSmoothGlow ? glowVisibility : outlineVisibility));
								if (hasTexture) {
									Texture texture = mat.mainTexture;
									fxMat.mainTexture = texture;
									fxMat.mainTextureOffset = mat.mainTextureOffset;
		...
		private static readonly int OverlayBackColorPropID = Shader.PropertyToID("_OverlayBackColor");
		private static readonly int CutOffPropID = Shader.PropertyToID("_CutOff");
		private static readonly int InnerGlowZTestPropID = Shader.PropertyToID("_InnerGlowZTest");
		private static readonly int WidthPropID = Shader.PropertyToID("_Width");
		private static readonly int ZTestPropID = Shader.PropertyToID("_ZTest");
		private static readonly int CullPropID = Shader.PropertyToID("_Cull");
		private static readonly int SeeThroughPropID = Shader.PropertyToID("_SeeThrough");
		private static readonly int SeeThroughNoisePropID = Shader.PropertyToID("_SeeThroughNoise");
		private static readonly int SeeThroughTintColorPropID = Shader.PropertyToID("_SeeThroughTintColor");
		private static readonly int SeeThroughBorderWidthPropID = Shader.PropertyToID("_SeeThroughBorderWidth");
		private static readonly int SeeThroughBorderConstantWidthPropID = Shader.PropertyToID("_SeeThroughBorderConstantWidth");
		private static readonly int BlurScalePropID = Shader.PropertyToID("_BlurScale");
		private static readonly int SpeedPropID = Shader.PropertyToID("_Speed");
		private static readonly int OutlineWidthPropID = Shader.PropertyToID("_OutlineWidth");
		private static readonly int OutlineZTestPropID = Shader.PropertyToID("_OutlineZTest");
		private static readonly int OutlineDirectionPropID = Shader.PropertyToID("_OutlineDirection");
		private static readonly int ConstantWidthPropID = Shader.PropertyToID("_ConstantWidth");
		private static readonly int GlowPropID = Shader.PropertyToID("_Glow");
		private static readonly int GlowColorPropID = Shader.PropertyToID("_GlowColor");
		private static readonly int GlowDirectionPropID = Shader.PropertyToID("_GlowDirection");
		private static readonly int Glow2PropID = Shader.PropertyToID("_Glow2");
		private static readonly int GlowZTestPropID = Shader.PropertyToID("_GlowZTest");
		...
		private static readonly int DebugPropID = Shader.PropertyToID("_Debug");
		private static readonly int FlipPropID = Shader.PropertyToID("_Flip");
		private static readonly int StereoRenderingPropID = Shader.PropertyToID("_StereoRendering");
		private static readonly int OutlineColorPropID = Shader.PropertyToID("_OutlineColor");
		private static readonly int OverlayDataPropID = Shader.PropertyToID("_OverlayData");
