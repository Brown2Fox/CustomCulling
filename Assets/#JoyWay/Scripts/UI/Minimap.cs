...
public class Minimap : Singleton<Minimap>
{
	...
	[Header("Image")]
	public int scaleFactor		= 1;                  // ƒл€ отрисовки карты, чем он больше, тем четче картинка
	...
	public struct MinimapElement
	{
		...
		// ѕровер€ем наход€тс€ ли координаты в пределах комнаты
	...
	public void Proccess() {
		...
		// Ќаходим самые отрицательные координаты
		...
		foreach (GeneratedRoom room in mapGenerator.generated) {
			...
			// «адаем его размер, положение и поворот
			...
			// ƒобавл€ем в список сгенеренных кусков карты
	...
	// ќтслеживаем положение руки чтобы скрывать/открывать карту
	...
	// ќтслеживаем какие чанки должны открытьс€ 
	...
	private void CheckNearChanks(RoomController chank) {
		...
		foreach (WayConnection one in chank.roomData.ways) {
			...
			from.x += chank.transform.position.x - ((chank.roomData.size.x - 1) * 8);       // ќтсчет идет от левого нижнего квадрата комнаты (если еЄ размер больше 1),
			from.y += chank.transform.position.z - ((chank.roomData.size.y - 1) * 8);       //  если она 1 то от этой самой комнаты

			Vector2 to = (one.offsetTo * 16);               // Ќам нужен двойной чтобы было не на границе чанков а в глубь следующего
	...
	// –ассчитываем новое положение миникарты (она центрируетс€ учитыва€ открытые чанки)
	...
	// –ассчитываем новый скейл миникарты
	...
	// ќтслеживаем положение игрока на карте и двигаем/вращаем его курсор
	...
	// –исует на текстуре чанк
	private void DrawImageNew(MinimapElement me) {

		// –ассчитываем шаг с которым будем брать цвет пиксел€ у оригинальной картинки
		int difW = (int)(me.source.width / (me.sizePixelPic.x * scaleFactor));
		int difH = (int)(me.source.height / (me.sizePixelPic.y * scaleFactor));

		// левый нижний угол данного чанка		≈го положение +	—амый отрицательный поо кориданатм ибо у текстуры нет отрицательной плоскости - половина его ширины/высоты
	...
	// ѕолучаем цвет с учетом поворота чанка
	...
	// ѕримен€ем нарисованное к текстуре
	private void Apply() {
		texture.Apply();
	}

	// ≈сли мы в туториале то карту не показываем
